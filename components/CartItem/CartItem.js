...
const CartItem = ({ item }) => {
  ...
  const handleIncrement = async () => {
    try {
      ...
      const userEmail = item.userEmail || localStorage.getItem('userEmail') || 'guest';
      dispatch(incrementQuantity(item.uniqueId, userEmail));
    } catch (error) {
  ...
  const handleDecrement = async () => {
    try {
      ...
      const userEmail = item.userEmail || localStorage.getItem('userEmail') || 'guest';
      dispatch(decrementQuantity(item.uniqueId, userEmail));
    } catch (error) {
  ...
  const handleRemove = async () => {
    try {
      ...
      const userEmail = item.userEmail || localStorage.getItem('userEmail') || 'guest';
      dispatch(removeFromCart(item.uniqueId, userEmail));
    } catch (error) {
